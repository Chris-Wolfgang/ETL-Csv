...
public static class CsvCheckpointExtensions
{
    ...
    /// <exception cref="FormatException">The file exists but does not contain a valid integer.</exception>
    public static async ValueTask<int> ReadCheckpointAsync(string path, CancellationToken token = default)
    {
        ...
        {
            // A checkpoint file is a single small integer; read it in one shot. Reading via the
            // stream (not StreamReader.ReadToEndAsync) lets us forward the token on every TFM.
    ...
    public static async ValueTask WriteCheckpointAsync(string path, int count, CancellationToken token = default)
    {
        ...
        // or a plain Move for the very first write.
